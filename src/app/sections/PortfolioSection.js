        ...
        <div className="relative w-full">
          <div className="relative h-144 md:h-96 xl:h-128 overflow-hidden rounded-lg">
            {portfolio.projects.map((project, index) => (
              <div key={project.name} className={`h-full absolute px-14 lg:px-16 transition-all duration-500 ${selected === index ? 'opacity-100' : `${selected > index ? '-' : ''}translate-x-full opacity-0`}`}>
          ...
          <div className="absolute z-30 flex space-x-3 -translate-x-1/2 left-1/2 bottom-4">
            {portfolio.projects.map((_, index) => (
              <button key={index} type="button" className={`w-3 h-3 rounded-full ${selected === index ? 'bg-indigo-500/60' : 'bg-gray-400/40'} hover:bg-indigo-500`} onClick={() => setSelected(index)}></button>
            ))}
          </div>
          <button type="button" className="absolute top-0 left-0 z-30 flex items-center justify-center h-full pl-2 cursor-pointer group focus:outline-none" onClick={onPrevious}>
          ...
          <button type="button" className="absolute top-0 right-0 z-30 flex items-center justify-center h-full pr-2 cursor-pointer group focus:outline-none" onClick={onNext}>
            <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-gray-200/80 dark:bg-gray-800/30 group-hover:bg-gray-200 dark:group-hover:bg-gray-800/60 group-focus:outline-none">
              <svg className="w-4 h-4 text-indigo-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
